...
public final class LetterUtils
{
    ...
    private static volatile LetterUtils letterUtils = new LetterUtils();
    ...
    public static LetterUtils getInstance()
    {

        if (letterUtils == null)
        {
            synchronized (LetterUtils.class)
            {
                if (letterUtils == null)
                {
     ...
     * 获取大写首字母
     */
    public char getInitial(String sourceStr)
    {
        char result = '\0';
        String str = sourceStr.toLowerCase(Locale.US);
     ...
     * @return 获得大写首字母，
     */
    public char getUpperCase(String string)
    {
        char res = '\0';
        String str = string.trim().toUpperCase(Locale.US);
        res = char2Initial(str.charAt(0));
        return res;
    }
